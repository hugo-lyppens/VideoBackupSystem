...
static BOOL FLB_ReadCylSec(int, char *, int, int, int);
static BOOL FLB_ReadNameDate(int, char *, struct DateStamp *);
static LONG FLB_StatusCmd(int, UWORD);


static char              FLB_drivename[][5] = {
      "DF0:", "DF1:", "DF2:", "DF3:"
};
...
struct DateStamp  *date;
{
   ...
   *name = 0;
   ...
   memcpy((char *)date, &buffer[512-7*4], 3*4);
   error = FALSE;
quit:
...
nextdisk:
   fl = FLB_drives[curdr];
   error = BAG_Search(BT_AMIGADOS, name, NULL);
   if(error)
      goto quit;
